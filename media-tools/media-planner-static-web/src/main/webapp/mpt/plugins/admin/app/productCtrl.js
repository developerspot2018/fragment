			...
			$scope.textAreaAdjust = function(o) {
			  ...
			  $scope.fetchAttributes = function(product_id){
				  ...
				  Data.get('products/'+product_id).then(function(result){
						...
						$scope.selectedProduct.attributeEntryLimit = 100; //max no of items to display in a page
...
//								  type: '',
			  ...
			  $scope.selectAttribute = function(attribute){
				  var index = 0; 
				  $scope.selectedProduct.attributeGrid.attributes[index].type = attribute.type;
				  $scope.selectedProduct.attributeGrid.attributes[index].description = attribute.description;
				  $scope.selectedProduct.attributeGrid.attributes[index].value = attribute.value;
				  $scope.selectedProduct.attributeGrid.attributes[index].id = attribute.id;
				  $scope.selectedProduct.attributeGrid.attributes[index].name = attribute.name;
			  }
			  ...
			  $scope.saveAttribute = function(data, attribute,self) {
				  ...
				  for (var attrIndex in $scope.selectedProduct.attributes) {
					  $scope.attrSet.push({"id" : $scope.selectedProduct.attributes[attrIndex].id});
				  }
				  
				  //$scope.selectedProduct.attributes = $scope.attrSet;
					  ...
					  if($scope.selectedProduct.attributeEdit){
						  ...
						  Data.put('products', postData).success(function(result){
							 ...
							 //$scope.selectedProduct.attributeGrid.attributes[index].name = attribute.name.name; 
							  ...
							  $scope.selectedProduct.creative = result.creative;
							 // $scope.selectedProduct.salesTargetList = backupSalesTarget;
							  ...
							  angular.copy($scope.attrSet, $scope.selectedProduct.attributes);
							  if(result.errorCode=='412') {
				            		$scope.showWarnigModal("This attribute is already associated . Please choose a different attribute to associate");
							  }
						  });
					  }
				  }
